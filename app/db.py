...
def create_job_row(job_id: str, username: str, filename: str, source_lang: str, target_lang: str, source_path: str) -> None:
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO jobs (id, username, filename, source_path, source_lang, target_lang, stage, progress, message, done, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, 'queued', 0, 'Queued', 0, ?, ?)",
            (job_id, username, filename, source_path, source_lang, target_lang, _now(), _now()),
        )


def fail_interrupted_jobs() -> int:
    """Mark jobs left active by a stopped process as retryable failures.

    ThreadPoolExecutor workers are in-process only, so a restart cannot
    continue a persisted `done = 0` job. Its saved pipeline artifacts remain
    intact for the retry/resume flow.
    """
    message = "Failed: application restarted before this job finished. Retry to resume from saved work."
    with get_connection() as conn:
        rows = conn.execute("SELECT id, logs FROM jobs WHERE done = 0").fetchall()
        for row in rows:
            logs = (row["logs"] or "").split("\n")
            logs.append(message)
            conn.execute(
                "UPDATE jobs SET stage = 'failed', done = 1, error = ?, message = ?, logs = ?, "
                "ended_at = ?, duration_seconds = NULL, updated_at = ? WHERE id = ?",
                (message, message, "\n".join(logs[-500:]), _now(), _now(), row["id"]),
            )
    if rows:
...
def list_job_rows(limit: int = 50, offset: int = 0, status: str | None = None, username: str | None = None) -> list[sqlite3.Row]:
    ...
    if status:
        if status == "running":
            ...
            clauses.append("stage = 'cancelled'")
    if username:
...
def job_stats() -> dict:
    with get_connection() as conn:
        ...
        cancelled = conn.execute("SELECT COUNT(*) AS c FROM jobs WHERE stage = 'cancelled'").fetchone()["c"]
    return {"total": total, "running": running, "completed": completed, "failed": failed, "cancelled": cancelled}
